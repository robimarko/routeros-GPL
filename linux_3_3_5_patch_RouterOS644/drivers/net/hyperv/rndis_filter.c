					     ...
					     struct rndis_message *resp)
{
	...
	if (indicate->status == RNDIS_STATUS_MEDIA_CONNECT) {
		netvsc_linkstatus_callback(
			net_device_ctx->device_ctx, 1);
	} else if (indicate->status == RNDIS_STATUS_MEDIA_DISCONNECT) {
		netvsc_linkstatus_callback(
			net_device_ctx->device_ctx, 0);
	} else {
		/*
		 * TODO:
		 */
				   ...
				   struct hv_netvsc_packet *pkt)
{
	...
	netvsc_recv_callback(net_device_ctx->device_ctx, pkt, csum_info);
}
...
{
	struct netvsc_device *nvdev = hv_device_to_netvsc_device(hdev);
	...
	if (set_complete->status != RNDIS_STATUS_SUCCESS) {
		netdev_err(hv_get_drvdata(hdev), "Fail to set MAC on host side:0x%x\n",
			   set_complete->status);
		ret = -EINVAL;
	}
				...
				struct ndis_offload_params *req_offloads)
{
	...
	*offload_params = *req_offloads;
	...
	set_complete = &request->response_msg.msg.set_complete;
	if (set_complete->status != RNDIS_STATUS_SUCCESS) {
		netdev_err(hv_get_drvdata(hdev), "Fail to set offload on host side:0x%x\n",
			   set_complete->status);
		ret = -EINVAL;
	}
				  ...
				  void *additional_info)
{
	...
	net_device->num_chn = min_t(u32, num_online_cpus(), VRSS_CHANNEL_MAX);
	net_device->num_chn = min_t(u32, net_device->num_chn, rsscap.num_recv_que);
