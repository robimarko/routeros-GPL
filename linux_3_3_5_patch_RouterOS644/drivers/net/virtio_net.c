...
static void virtnet_enable_mq(struct virtnet_info *vi) {
	int i, cpu, queue_pair_count;

	queue_pair_count = vi->max_queue_pairs;
	queue_pair_count = min_t(int, queue_pair_count, num_online_cpus());
	if (virtnet_set_queues(vi, queue_pair_count)) {
		printk(KERN_WARNING "failed to set queues\n");
		return;
	}

	i = 0;
	for_each_online_cpu(cpu) {
		if (i == queue_pair_count) break;
		virtqueue_set_affinity(vi->rq[i].vq, cpu);
		virtqueue_set_affinity(vi->sq[i].vq, cpu);
		*per_cpu_ptr(vi->vq_index, cpu) = i;
		i++;
...
static int virtnet_probe(struct virtio_device *vdev)
{
	...
	if (!dmi_match(DMI_BOARD_NAME, "VirtualBox"))
	...
	/* Always try to enable mq */
	if (vi->max_queue_pairs > 1)
		virtnet_enable_mq(vi);
