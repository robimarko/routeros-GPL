...
static struct proc_dir_entry *ath_usb_proc = NULL;
 ...
 * reference
 */
static const char driver_name[] = "rb400-ehci";
...
static void ath_usb_init_device(struct ath_usb_udc *udc)
{
	...
	if(is_wasp()) {
		...
		ath_reg_wr(ATH_DDR_CTL_BASE + 0xc4, 0x72224222);
		ath_reg_wr(ATH_DDR_CTL_BASE + 0xc8, 0x22224222);
	}
...
static int ath_usb_udc_init(struct ath_usb_udc *udc, struct device *dev)
{
	...
	/* Setup all endpoints */
	ath_usb_setup(udc);
	temp = device_add(&udc->gadget.dev);
	return temp;
}
...
static int ath_usb_udc_probe(struct platform_device *pdev)
{
	...
	reg_base = ioremap_nocache(pdev->resource[0].start,
			pdev->resource[0].end - pdev->resource[0].start + 1);
	if (!reg_base) {
...
static int __init ath_usb_init(void)
{
	...
	sys_regs = ioremap_nocache(0x18000000, 0x100000);
