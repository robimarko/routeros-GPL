...
struct al_crc_ctx {
	...
	enum al_crypto_crcsum_type	crcsum_type;
	uint32_t			key;
};
	...
	if (likely(!req_ctx->last)) {
		...
		if (!req_ctx->cache_state.cached) {
			xaction->cached_crc_indx = al_crypto_cache_replace_lru(
					...
					NULL);
				xaction->flags = AL_CRYPT_BARRIER;
		} else {
...
static int crc_process_req(struct ahash_request *req, unsigned int nbytes)
{
	...
	if (nbytes) {
		...
		dev_dbg(to_dev(chan), "%s: src_nents=%d, chained=%d\n",
				__func__, src_nents, src_chained);

		sgc = dma_map_sg_chained(to_dev(chan),
				req->src, src_nents,
		...
		rc = al_crypto_crc_csum_prepare(chan->hal_crypto, chan->idx,
