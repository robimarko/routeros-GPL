...
{
	clk_disable_unprepare_dk(ctrl->rsrcs.core_clk);
	clk_disable_unprepare_dk(ctrl->rsrcs.iface_clk);
}

static int i2c_msm_pm_clk_prepare_enable(struct i2c_msm_ctrl *ctrl)
{
	int ret;
	ret = clk_prepare_enable_dk(ctrl->rsrcs.iface_clk);
	if (ret) {
		dev_err(ctrl->dev,
	...
	ret = clk_prepare_enable_dk(ctrl->rsrcs.core_clk);
	if (ret) {
		clk_disable_unprepare_dk(ctrl->rsrcs.iface_clk);
		dev_err(ctrl->dev,
...
//				of_property_read_bool(node, itr->dt_name);
				of_find_property(node, itr->dt_name, NULL) ? true : false;
			ret = 0;
			break;
		case DT_ID:
...
static int i2c_msm_rsrcs_clk_init(struct i2c_msm_ctrl *ctrl)
{
	...
	ctrl->rsrcs.core_clk = of_clk_get_by_name(ctrl->dev->of_node, "core_clk");
	...
	ret = clk_set_rate_dk(ctrl->rsrcs.core_clk, ctrl->rsrcs.clk_freq_in);
	if (ret) {
	...
	ctrl->rsrcs.iface_clk = of_clk_get_by_name(ctrl->dev->of_node, "iface_clk");
