...
#define add_mtd_partitions(mtd, parts) \
...
static struct mtd_partition rt2880_small_partition[] = {
	{
		.name = "RouterBoot",
		.offset = 0,
		.size = 256 * 1024,
...
#define CONFIG_FB_MEDIATEK_TRULY
#define CONFIG_FB_MEDIATEK_ILITEK
...
static struct chip_info chips_data [] = {
	...
	{ "W25X40CL",           0xef, 0x30130000, 64 * 1024,  8,  0 },
	...
	{ "ABSENT",	        0xff, 0xffffffff, 0, 0, 0 },
	{ "ABSENT",	        0x00, 0x00000000, 0, 0, 0 },
...
#if defined (CONFIG_RALINK_MT7621) && (defined (CONFIG_FB_MEDIATEK_TRULY) || defined(CONFIG_FB_MEDIATEK_ILITEK))
	bbu_spic_busy_wait();
  sysRegWrite(SPI_REG_MASTER, (ra_inl(SPI_REG_MASTER) & BIT(29)) | 0x58880);
#endif	  
...
int ralink_spi_trans(const u8 cs, const u8 cmd_len, const u32 cmd,
		const u8 d_len, u8* d_buf) {
	int rc = -1;
	u32 tmpCtl, tmpMaster, tmpOpcode, tmpMoreBuf, d[4] = {0, 0, 0, 0};
	if (cs > 7) {
		return -1;
	}
	if (cmd_len > 4) {
	...
	spin_lock(&flash_lock);
#endif
	tmpCtl = ra_inl(SPI_REG_CTL);
	tmpMaster = ra_inl(SPI_REG_MASTER);
	tmpOpcode = ra_inl(SPI_REG_OPCODE);
	tmpMoreBuf = ra_inl(SPI_REG_MOREBUF);

	sysRegWrite(SPI_REG_MASTER, 0
		| ((u32)cs << 29)
		| (0UL << 28)	// period of SCLL HIGH is longer
		| (160UL << 16)	// SPI clk divider, SPI clock ~1.35Mhz
		| (31UL << 11)	// min CS deselect clock cycles
		| (1UL << 10)	// full fuplex
		| (0UL << 9)	// int disabled
		| (1UL << 8)	// 6clk between cs and sclk
		| (0UL << 7)	// spi pre-fetch disabled
		| (0UL << 6)	// both MISO and MOSI pins used
		| (0UL << 5)	// CPHA
		| (0UL << 4)	// CPOL
		| (0UL << 3)	// MSB first
		| (1UL << 2)	// MORE BUFF MODE
		| (0UL << 0)	// serial mode normal
	);
	sysRegWrite(SPI_REG_MOREBUF, 0
		| (((u32)cmd_len * 8) << 24)	// CMD bit count
		| (((u32)d_len * 8) << 12)	// MISO bit count
		| (((u32)d_len * 8) << 0)	// MOSI bit count
	);

	rc = bbu_spic_busy_wait();
	if (rc >= 0) {
		if (d_buf) {
			u8 *pd = (u8 *)d;
			int i;
			for (i = d_len - 1; i >= 0; --i) {
				...
				if (i % 4 == 0) {
					u8 r = i / 4;
					ra_outl(SPI_REG_DATA(r), d[r]);
				}
			}
		}
		sysRegWrite(SPI_REG_OPCODE, cmd);
		sysRegWrite(SPI_REG_CTL, SPI_CTL_START);
		rc = bbu_spic_busy_wait();
	}
	if (rc >= 0) {
		if (d_buf) {
	...
	sysRegWrite(SPI_REG_MOREBUF, tmpMoreBuf);
	sysRegWrite(SPI_REG_OPCODE, tmpOpcode);
	sysRegWrite(SPI_REG_MASTER, tmpMaster);
	sysRegWrite(SPI_REG_CTL, tmpCtl);
#if defined (CONFIG_RALINK_MT7621) && defined (CONFIG_FB_MEDIATEK_TRULY)
	spin_unlock(&flash_lock);
#endif
	up(&flash->lock);
...
#elif defined BBU_MODE
	retval = bbu_spic_trans(code, 0, rxbuf, 1, 4, SPIC_READ_BYTES, FLASH_USE);
	if (!retval)
		retval = n_rx;
#endif
	if (retval != n_rx) {
...
static int ramtd_read_fact(struct mtd_info *mtd, loff_t from, size_t len,
	size_t *retlen, u_char *buf)
{
	int rc;
	char cmd;
	...
	if (from == 0) {
		...
		int res;
		memset(buf, 0, len);
		res = raspi_read_devid(buf, len);
		if (res < 0) {
			printk("ramtd_read_fact ERROR: id read failed\n");
			return -EIO;
		}
		*retlen = res;
		return 0;
	...
	if (from != 4 || len > 32) {
		printk("ramtd_read_fact ERROR: from (%d) != 4"
		       " or rx len (%d) > 32\n",
		       (int)from, len);
		return -EINVAL;
	}
	...
	cmd = buf[0];
	from = ((unsigned)buf[1] << 16) | ((unsigned)buf[2] << 8) | buf[3];
	memset(buf, 0, len);
	rc = bbu_mb_spic_trans(cmd, from, buf, 0, len, SPIC_READ_BYTES, FLASH_USE);

	if (rc != 0) {
		printk("ramtd_read_fact ERROR: custom read failed\n");
		return -EIO;
	}
	*retlen = len;
	return 0;
...
static int ramtd_erase_1(struct mtd_info *mtd, struct erase_info *instr)
{
	int ret;
	down(&flash->lock);
...
static int ramtd_read_1(struct mtd_info *mtd, loff_t from, size_t len,
		        size_t *retlen, u_char *buf)
{
	int ret;
	down(&flash->lock);
...
static int ramtd_write_1(struct mtd_info *mtd, loff_t to, size_t len,
		        size_t *retlen, const u_char *buf)
{
	int ret;
	down(&flash->lock);
...
static int ramtd_read_fact_1(struct mtd_info *mtd, loff_t from, size_t len,
	size_t *retlen, u_char *buf)
{
	int ret;
	down(&flash->lock);
	ret = ramtd_read_fact(mtd, from, len, retlen, buf);
	up(&flash->lock);
	return ret;
}
...
static int ramtd_read_fact_2(struct mtd_info *mtd, loff_t from, size_t len,
	size_t *retlen, u_char *buf)
{
	int ret;
	down(&flash->lock);
...
//	}
	...
	ra_and(RALINK_SYSCTL_BASE + 0x60, ~(3 << 16));
        ra_or(SPI_REG_MASTER, (1 << 29));
#endif
	...
	flash->mtd.writebufsize = 256; /* usual page size */

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,3,5)
	flash->mtd.erase = ramtd_erase_1;
	flash->mtd.read = ramtd_read_1;
	flash->mtd.write = ramtd_write_1;
	flash->mtd.lock = ramtd_lock;
	flash->mtd.unlock = ramtd_unlock;
	flash->mtd.read_fact_prot_reg = ramtd_read_fact_1;
	...
	ramtd_read_1(NULL, offs, sizeof(hdr), (size_t *)&i, (u_char *)(&hdr));
	if (hdr.ih_ksz != 0) {
	...
	flash->mtd2.writebufsize = 256; /* usual page size */
	flash->mtd2.read_fact_prot_reg = ramtd_read_fact_2;
	...
	if (flash->mtd.size >= 0x1000000) {
	    add_mtd_partitions(&flash->mtd, rt2880_partitions);
	}

	if (flash->mtd2.size >= 0x1000000) {
	    rt2880_partitions[1].name = "RouterBootFake";
	    add_mtd_partitions(&flash->mtd2, rt2880_partitions);
	}

	if (flash->mtd2.size > 0 && flash->mtd.size < 0x1000000) {
	    add_mtd_partitions(&flash->mtd, rt2880_small_partition);
	}

	if (flash->mtd2.size > 0 && flash->mtd2.size < 0x1000000) {
	    add_mtd_partitions(&flash->mtd2, rt2880_small_partition);
	}
	return NULL;
#else
...
EXPORT_SYMBOL(ralink_spi_trans);
MODULE_LICENSE("GPL");
