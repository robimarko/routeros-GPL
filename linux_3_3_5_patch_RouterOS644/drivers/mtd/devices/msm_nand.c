...
#if DEBUG_CUSTOM_CMD
static void dumpRegs(unsigned char *buf, unsigned len, const char *name) {
    int i;
    int eq = 0;
...
#if DEBUG_CUSTOM_CMD
				pr_info("ONFI optional cmds: %x\n",
					onfi_param_page_ptr->optional_commands_supported);
				dumpRegs(onfi_param_page_ptr, 256, "ONFI Parameter Page");
#endif
	...
	if (ops->datbuf) {
		data_dma_addr_curr = data_dma_addr =
			msm_nand_dma_map(chip->dev, ops->datbuf, ops->len,
					 DMA_BIDIRECTIONAL, &dat_bounce_buf);
...
static void msm_cfg_ctrl(struct msm_nand_chip *chip, unsigned int ctrl) {
	unsigned ncfg = NCFG_DEF;
	if (ctrl & NAND_NCE) ncfg ^= (1 << 4);
	if (ctrl & NAND_CLE) ncfg ^= (1 << 2);
	if (ctrl & NAND_ALE) ncfg ^= (1 << 3);
#if DEBUG_CUSTOM_CMD
	printk("msm_cfg_ctrl%s%s%s\n",
	       (ctrl & NAND_NCE) ? " NCE" : "",
	       (ctrl & NAND_CLE) ? " CLE" : "",
	       (ctrl & NAND_ALE) ? " ALE" : "");
#endif
	flash_wr_reg(chip, MSM_NAND_CONFIG, ncfg);
}

static void msm_cfg_write(struct msm_nand_chip *chip, unsigned int cmd) {
	unsigned ncfg_orig = flash_rd_reg(chip, MSM_NAND_CONFIG);
	unsigned ncfg = ncfg_orig;
	ncfg |= (cmd << 16) | (1 << 13);
	ncfg ^= (1 << 1);	// nWE <- low
	flash_wr_reg(chip, MSM_NAND_CONFIG, ncfg);
	ncfg ^= (1 << 1);	// nWE <- high
	flash_wr_reg(chip, MSM_NAND_CONFIG, ncfg);
	flash_wr_reg(chip, MSM_NAND_CONFIG, ncfg_orig);
#if DEBUG_CUSTOM_CMD
	printk("msm_cfg_write %x (cfg %x)\n", cmd, ncfg);
#endif
}

static void msm_cfg_ctrl_write(struct msm_nand_chip *chip,
			       unsigned int ctrl,
			       unsigned int cmd) {
	msm_cfg_ctrl(chip, ctrl);
	msm_cfg_write(chip, cmd);
}

static u_char msm_cfg_read(struct msm_nand_chip *chip) {
	unsigned ncfg_orig = flash_rd_reg(chip, MSM_NAND_CONFIG);
	unsigned ncfg = ncfg_orig ^ (1 << 0);	// nRE <- low
	flash_wr_reg(chip, MSM_NAND_CONFIG, ncfg);
	ncfg ^= (1 << 10);	// sample read data
	flash_wr_reg(chip, MSM_NAND_CONFIG, ncfg);
	flash_wr_reg(chip, MSM_NAND_CONFIG, ncfg_orig);
#if DEBUG_CUSTOM_CMD
	u_char val = flash_rd_reg(chip, MSM_NAND_CONFIG_DATA);
	printk("msm_cfg_read %x (cfg %x)\n", (unsigned)val, ncfg);
	return val;
#else
	return flash_rd_reg(chip, MSM_NAND_CONFIG_DATA);
#endif
}

static int msm_nand_read_fact(struct mtd_info *mtd, loff_t from,
	...
	/* enter configuration access mode */
	flash_wr_reg(chip, MSM_NAND_CONFIG, NCFG_DEF);
	flash_wr_reg(chip, MSM_NAND_CONFIG_MODE, 1);
	while (!(flash_rd_reg(chip, MSM_NAND_CONFIG_STATUS) & 1)) {}

	if (from == 0) {
		/* read nand id */
		msm_cfg_ctrl_write(chip, NAND_NCE | NAND_CLE, NAND_CMD_READID);
		msm_cfg_ctrl_write(chip, NAND_NCE | NAND_ALE, 0);
	}
	else {
		for (ptr = buf; *ptr != 0; ptr += 2) {
			...
			if (cmd) msm_cfg_write(chip, cmd);
		}
		++ptr;
		msm_cfg_ctrl_write(chip, NAND_NCE | NAND_CLE, *ptr);
		msm_cfg_ctrl_write(chip, NAND_NCE | NAND_ALE, *(ptr + 2));
		msm_cfg_write(chip, 0);			/* column byte 2 */
		msm_cfg_write(chip, *(ptr + 1));	/* page */
		msm_cfg_write(chip, 0);
		if (supported_flash.density > (128 << 20)) {
			msm_cfg_write(chip, 0);
		}
		if (*ptr == NAND_CMD_READ0) {
			msm_cfg_ctrl_write(chip, NAND_NCE | NAND_CLE,
					   NAND_CMD_READSTART);
		}
	}
	msm_cfg_ctrl(chip, NAND_NCE);

	/* wait for nand ready */
	while (!(flash_rd_reg(chip, MSM_NAND_FLASH_STATUS) & (1 << 5)));

	for (i = 0; i < len; ++i) {
		buf[i] = msm_cfg_read(chip);
	}
	*retlen = len;

	/* exit configuration access mode */
	flash_wr_reg(chip, MSM_NAND_CONFIG_MODE, 0);
	while (flash_rd_reg(chip, MSM_NAND_CONFIG_STATUS) & 1) {}
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	mtd->read_fact_prot_reg = msm_nand_read_fact;
...
#endif
#ifdef CONFIG_MTD_NAND_RB
	rb_nand_register_partitions(&info->mtd);
#else
