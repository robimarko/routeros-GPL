...
//		ret = msm_nand_bus_set_vote(info, 1);
		...
		ret = clk_prepare_enable_dk(info->clk_data.qpic_clk);
		if (ret) {
...
//			msm_nand_bus_set_vote(info, 0);
		...
		clk_disable_unprepare_dk(info->clk_data.qpic_clk);
//		msm_nand_bus_set_vote(info, 0);
	}
...
//	if (err)
//		goto out;
	info->clk_data.qpic_clk = of_clk_get_by_name(pdev->dev.of_node, "core_clk");
	if (!IS_ERR_OR_NULL(info->clk_data.qpic_clk)) {
		err = clk_set_rate_dk(info->clk_data.qpic_clk,
...
//		goto bus_unregister;
	...
	ppdata.of_node = pdev->dev.of_node;
	mtd_device_parse_register(&info->mtd, NULL, &ppdata, NULL, 0);
#endif
	goto out;
free_bam:
