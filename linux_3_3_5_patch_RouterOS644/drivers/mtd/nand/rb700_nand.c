...
static struct spi_message m_acquire;
static struct spi_message m_release;
...
static void rb700_gpio_init(void) {
	...
	/* ALE is being set low during chip select */
...
static void rb700_select_chip(struct mtd_info *mtd, int chipnr)
{
	...
	if (chipnr >= 0) {
		...
		GPIO_REG(GPIO_OE_REG) &= ~(GPI_RB700_NAND_RDY |
					   GPO_NAND_DATA(0xff));
		GPIO_REG(GPIO_S0_REG) = GPO_RB700_NAND_ALE;
...
static int rb700_spi_probe(struct spi_device *_spi) {
	static struct spi_transfer st_ack = {
		.len = 0,
		.cs_change = 1,
	};
	printk("RB700 spi nand\n");
	spi = _spi;
	spi_message_init(&m_acquire);
	spi_message_add_tail(&st_ack, &m_acquire);
	spi_message_init(&m_release);
	return 0;
}

static int rb700_spi_remove(struct spi_device *_spi) {
	spi = NULL;
	return 0;
}

static struct spi_driver rb700_nand_spi = {
	.driver	= {
		.name = "rb700-spi-nand",
		.owner = THIS_MODULE,
	},
	.probe	= rb700_spi_probe,
	.remove = rb700_spi_remove,
};
...
static int __init rb700_nand_init(void)
{
	...
	if (mips_machtype == MACH_MT_SXT_LTE) {
	    rb700_change_gpo(BIT(17), 0);
	}

	spi_register_driver(&rb700_nand_spi);
...
static void __exit rb700_nand_exit(void)
{
	...
	spi_unregister_driver(&rb700_nand_spi);
}
