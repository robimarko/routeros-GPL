...
static unsigned	GPO_NAND_nCE = (1 << 14);
...
static unsigned char *verify_buf;
...
{
	BUG_ON(len > VERIFY_BUF_SIZE);
	ar9344_read_buf(mtd, verify_buf, len);

	if (memcmp(buf, verify_buf, len) != 0) {
#ifdef AR9344_NAND_DEBUG
		int i;
		for (i = 0; i < len; ++i) {
			if (verify_buf[i] != buf[i]) {
				printk("data[%d]: wanted %02x got %02x\n",
				       i, (unsigned)buf[i], (unsigned)verify_buf[i]);
			}
		}
#endif
		return -EFAULT;
			   ...
			   int column, int page)
{
	...
	if (command == NAND_CMD_READID) {
		read_buf_from_nand(0, column, 16, 0x9061);
		dma_buf_ofs = column;
		return;
	}
	if (command == NAND_CMD_PARAM) {
		read_buf_from_nand(0, column, 2112, 0xec62);
		dma_buf_ofs = column;
		return;
	}
	if (command == NAND_CMD_ONFI_READUID) {
		read_buf_from_nand(0, column, 512, 0xed62);
		dma_buf_ofs = column;
		return;
	}
	if (!ctrl_init_done) {
...
static int ar9344_nand_probe(struct platform_device *pdev)
{
	...
	verify_buf = kmalloc(VERIFY_BUF_SIZE, GFP_KERNEL);
	if (!verify_buf) {
		return -ENOMEM;
	}
...
{
	unsigned nand_addr = NAND_FLASH_BASE;
	extern int is_scorpion(void);
	if (is_scorpion()) {
		nand_addr += 0x00800000;
		GPO_NAND_nCE = (1 << 23);
	}
	nand_base = ioremap_nocache(nand_addr, PAGE_SIZE);    
