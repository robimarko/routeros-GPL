 ...
 * read len bytes from the nand device.
 */
int nand_verify_buff(struct mtd_info *mtd, const uint8_t *buf, int len)
{
	uint8_t rbuf[len];
	nand_read_buff(mtd, rbuf, len);
	return memcmp(rbuf, buf, len) ? -EFAULT : 0;
}
...
int nand_init_size(struct mtd_info *mtd, struct nand_chip *chip, u8 *id_data)
{
	int extid;
	...
	/* setup minimum amount of ONFI settings with some valid values */
	chip->onfi_params.async_timing_mode = 0x1f;
	chip->onfi_params.addr_cycles = 0x22;		// 2 column, 2 row bytes
	if (chip->chipsize > (128 << 20)) {
		++chip->onfi_params.addr_cycles;	// 3 rows
	}
	chip->onfi_params.byte_per_page = cpu_to_le32(mtd->writesize);
	chip->onfi_params.ecc_bits = 1;

	return 0;	/* only 8 bit bus is supported */
}
...
static int al_nand_probe(struct platform_device *pdev)
{
	...
	nand->verify_buf = nand_verify_buff;
