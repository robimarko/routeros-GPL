...
static int use_clocks(void) {
	return 0;
}

static long clk_default_rate(void) {
	return 1843200;
}
...
static void msm_shutdown(struct uart_port *port)
{
	...
	if (use_clocks()) clk_disable_unprepare(msm_port->clk);
...
static int disable_uart(char *opt)
{
	uart_disabled = 1;
	return 0;
}
...
static int msm_serial_probe(struct platform_device *pdev)
{
	...
	if (use_clocks()) {
		...
		if (msm_port->is_uartdm) {
			...
			clk_set_rate(msm_port->clk, clk_default_rate());
		}
