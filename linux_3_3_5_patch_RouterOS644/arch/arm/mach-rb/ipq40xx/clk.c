...
/**
 * clk_unregister_dk - unregister a currently registered clock
...
{
	clk_unregister_dk(*(struct clk **)res);
}
...
struct clk *of_clk_get_by_clkspec(struct of_phandle_args *clkspec)
{
	...
	if (!IS_ERR(clk) && !__clk_get_dk(clk))
		clk = ERR_PTR(-ENOENT);
...
const struct of_device_id *get_clk_of_table(void) {
	return &__clk_of_table;
}
		...
		unsigned long fixed_rate, unsigned long fixed_accuracy)
{
	...
	/* register the clock */
	clk = clk_register_dk(dev, &fixed->hw);
	if (IS_ERR(clk))
		kfree(fixed);
