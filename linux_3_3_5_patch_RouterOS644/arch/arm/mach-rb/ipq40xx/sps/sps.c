...
// does not perform all sorts of pipe checks and calls transfer directly
// also disables irqs only on last iovec which touches pipe's desc offset register
int sps_transfer_fast(struct sps_pipe *pipe, struct sps_transfer *transfer)
{
        const int pipe_index = pipe->pipe_index;
	...
	for (n = (int)transfer->iovec_count - 1, iovec = transfer->iovec;
		...
		if (n > 0) {
			...
			flags = iovec->flags;
			user = transfer->user;
                        local_irq_save(irqsave_flags);
                        irq_restore = 1;
		}
		result = sps_bam_pipe_transfer_one(bam, pipe_index,
						 iovec->addr,
						 iovec->size, user,
						 flags);
        ...
        if (irq_restore) local_irq_restore(irqsave_flags);

	return result;
}
EXPORT_SYMBOL(sps_transfer_fast);
