...
//	{768000000, DDRC_PLL_666, 0x2, 0, 0},
//	{823000000, DDRC_PLL_666, 0x1, 0, 0},
//	{896000000, DDRC_PLL_666, 0x0, 0, 0},
//	{}
//};
static struct freq_tbl ftbl_gcc_apps_clk[0xd]; 
static int cpu_pll_dividers[0xd] = {/*26,*/ 24, 22, 21, 20, 19, 18, 17, 16, 15, 14, 13, 12}; 


static void fill_cpu_freq_table(void) 
{ 
	int i; 
	unsigned fdbck, refclk, reg_val, freq, vco; 
	void *address; 

	address = ioremap(0x182e020, 0x4); 
	reg_val = ioread32(address); 
	iounmap(address); 
	fdbck = (reg_val & (0xff << 0x10)) >> 0x10; 
	refclk = (reg_val & (0x1f << 0x18)) >> 0x18; 

	vco = (48000 * 2 * fdbck) / refclk; 
	for (i = 0; i < 0xc; i++) { 
		freq = (vco * 2) / cpu_pll_dividers[i];
		freq /= 1000;
		ftbl_gcc_apps_clk[i].freq = freq * 1000000;
		ftbl_gcc_apps_clk[i].src = DDRC_PLL_666; 
		ftbl_gcc_apps_clk[i].pre_div = 0xb - i;
	} 
} 

unsigned long ipq40xx_get_freq_index(int index)
{
	return ftbl_gcc_apps_clk[index].freq;
}
...
static const struct regmap_config gcc_ipq40xx_config = {
	.reg_bits	= 32,
	.val_bits	= 32,
...
MODULE_DEVICE_TABLE(of, gcc_ipq40x_match_table);
...
static int gcc_ipq40xx_probe(struct platform_device *pdev)
{
	...
	/* XTAL and RTC */
	clk_register_fixed_rate(dev, "xo", NULL, CLK_IS_ROOT,
				      48000000);
	clk_register_fixed_rate(dev, "gcc_sleep_clk_src", NULL, CLK_IS_ROOT,
				      32000);
//	clk_register_fixed_rate(dev, "wifi_rtc_clk_src", NULL, CLK_IS_ROOT,
//				      32768);
