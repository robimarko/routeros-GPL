...
int clk_prepare_ak(struct clk *clk)
{
	...
	if (clk->prepare_count == 0) {
		...
		ret = clk_prepare_ak(parent);
		if (ret)
			goto out;
		ret = clk_prepare_ak(clk->depends);
		if (ret)
...
EXPORT_SYMBOL(clk_unprepare_ak);
...
EXPORT_SYMBOL(clk_round_rate_ak);
...
EXPORT_SYMBOL(clk_set_parent_ak);
...
EXPORT_SYMBOL(clk_get_parent_ak);
