...
#define IRQ_AURORA_MSI_START		(IRQ_AURORA_GPIO_START + 34)
...
armada_370_xp_handle_irq(struct pt_regs *regs)
{
	...
	do {
		...
		if (irqnr > 1) {
			...
			//		  irqnr, regs);
                        //irqnr = irq_domain_to_irq(&armada_370_xp_mpic_domain, irqnr);
					     ...
					     struct device_node *parent)
{
	...
	int nr_irqs, i;
        unsigned int irq;
	u32 control;
        struct irq_domain *domain = &armada_370_xp_mpic_domain;
	...
	domain->nr_irq = IRQ_AURORA_MSI_START;
	domain->irq_base = 0; // descs are allocated in early_irq_init
        domain->hwirq_base = 0;
        irq_domain_add(domain);

	irq_domain_for_each_irq(domain, i, irq) {
                if (irq < IRQ_MAIN_INTS_NUM) {
                    armada_370_xp_irq_mask(irq_get_irq_data(irq));
                }

		irq_set_chip(irq, &armada_370_xp_irq_chip);
		irq_set_chip_data(irq, 0);
		irq_set_handler(irq, handle_level_irq);
		irq_set_status_flags(irq, IRQ_LEVEL);
		if (irq < MAX_PER_CPU_IRQ_NUMBER && irq != IRQ_AURORA_MP) {
			irq_set_chip_and_handler(irq, &armada_370_xp_irq_chip,
						 handle_percpu_devid_irq);
...
//					armada_370_xp_mpic_handle_cascade_irq);
	...
	for (irq = 0; irq < IRQ_AURORA_MSI_START ; irq++) {
		unsigned reg;
		reg = readl(main_int_base + ARMADA_370_XP_INT_SOURCE_CTL(irq));
		reg = (reg & (~ARMADA_370_XP_INT_SOURCE_CPU_MASK)) | BIT(0);
		writel(reg, main_int_base + ARMADA_370_XP_INT_SOURCE_CTL(irq));
        }
...
struct of_device_id msys_irq_match[] = {
