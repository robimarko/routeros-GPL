 ...
 * raw hw_event ID.
 */
typedef int cache_map_t[PERF_COUNT_HW_CACHE_MAX]
...
struct tile_pmu {
	...
	const cache_map_t *cache_events; /* generic hw cache events table */
...
static const  cache_map_t tile_cache_event_map = {
[C(L1D)] = {
	[C(OP_READ)] = {
...
EXPORT_SYMBOL(handle_syscall_link_address);
