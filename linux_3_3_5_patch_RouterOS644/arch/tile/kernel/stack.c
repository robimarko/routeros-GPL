			  ...
			  int have_mmap_sem, char *buf, size_t bufsize)
{
	...
	if (address >= PAGE_OFFSET) {
		...
		rc = snprintf(p, remaining, "+%#lx/%#lx ",
			      offset + adjust, size);
		if (mod && rc < remaining)
			snprintf(p + rc, remaining - rc, "[%s] ", mod->name);
		buf[bufsize-1] = '\0';
		return;
	}
 ...
 */
DEFINE_SPINLOCK(stack_dump_lock);
...
void tile_show_stack(struct KBacktraceIterator *kbt, int headers)
{
	...
	spin_lock_irqsave(&stack_dump_lock, flags);
	...
	for (; !KBacktraceIterator_end(kbt); KBacktraceIterator_next(kbt)) {
		...
		pr_err("  frame %d: 0x%lx %pS (sp 0x%lx)\n",
			i++, address, (void *)address, (unsigned long)(kbt->it.sp));
	...
	spin_unlock_irqrestore(&stack_dump_lock, flags);
	if (have_mmap_sem)
