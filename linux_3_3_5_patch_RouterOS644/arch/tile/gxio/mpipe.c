...
#define mpipe_mode_flags 0

#define MUTEX_LOCK(_mutexp)  mutex_lock(_mutexp)
#define MUTEX_UNLOCK(_mutexp)  mutex_unlock(_mutexp)
...
EXPORT_SYMBOL_GPL(__gxio_mpipe_init);

// Support 4.1.4 and earlier ABI for backwards binary compatibility
int gxio_mpipe_init_414(gxio_mpipe_context_t *context,
				   ...
				   unsigned int flags)
{
	...
	retval = gxio_mpipe_alloc_buffer_stacks_aux(context, count, first,
						    flags);
...
EXPORT_SYMBOL_GPL(gxio_mpipe_alloc_buffer_stacks);
...
int gxio_mpipe_link_mdio_rd_ex(gxio_mpipe_link_t *link, int phy, int dev,
			       int addr)
{
	return gxio_mpipe_link_mdio_rd_aux(link->context, link->mac, phy, dev,
					   addr);
}

int gxio_mpipe_link_mdio_wr_ex(gxio_mpipe_link_t *link, int phy, int dev,
...
int gxio_mpipe_link_mdio_rd(gxio_mpipe_link_t *link, int dev, int addr)
{
	return gxio_mpipe_link_mdio_rd_aux(link->context, link->mac, -1, dev,
					   addr);
}

int gxio_mpipe_link_mdio_wr(gxio_mpipe_link_t *link, int dev, int addr,
			    uint16_t val)
{
...
int64_t gxio_mpipe_link_get_attr(gxio_mpipe_link_t *link, uint32_t attr)
{
	int64_t data;
	int rv;
	...
	rv = gxio_mpipe_link_get_attr_aux(link->context,
					  (link->
					   mac << 24) | (attr & 0xFFFFFF),
					  &data);
	return (rv < 0) ? rv : data;
}

int gxio_mpipe_link_arm_pollfd(gxio_mpipe_context_t *context, int fd)
{
	return gxio_mpipe_arm_pollfd(context, fd);
}

int gxio_mpipe_link_close_pollfd(gxio_mpipe_context_t *context, int fd)
{
