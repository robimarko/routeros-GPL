...
extern int __gxio_mpipe_init(gxio_mpipe_context_t *context,
...
static inline int gxio_mpipe_init(gxio_mpipe_context_t *context,
				  unsigned int mpipe_instance)
{
	int retval;
...
static inline gxio_mpipe_bdesc_t
 ...
 *  will be sent until gxio_mpipe_enable_notif_ring_interrupt() is called.
 *
 * @param context An initialized mPIPE context.
 ...
 */
extern int gxio_mpipe_equeue_set_priority(gxio_mpipe_equeue_t *equeue,
					  gxio_mpipe_equeue_priority_enum_t
					  priority);
 ...
 */
extern int gxio_mpipe_equeue_get_priority(gxio_mpipe_equeue_t *equeue);
 ...
 * @param stack The buffer stack index.
 */
extern int gxio_mpipe_get_buffer_count(gxio_mpipe_context_t *context,
				       unsigned int stack);
 ...
 */
extern int gxio_mpipe_link_mdio_rd(gxio_mpipe_link_t *link, int dev, int addr);
 ...
 */
extern int gxio_mpipe_link_mdio_wr(gxio_mpipe_link_t *link, int dev, int addr,
				   uint16_t val);
 ...
 *  or a negative error code.
 */
extern int gxio_mpipe_link_mdio_rd_ex(gxio_mpipe_link_t *link, int phy,
				      int dev, int addr);
 ...
 * @return Zero if the write was successful, or a negative error code.
 */
extern int gxio_mpipe_link_mdio_wr_ex(gxio_mpipe_link_t *link, int phy,
 ...
 */
extern int64_t gxio_mpipe_link_get_attr(gxio_mpipe_link_t *link,
					uint32_t attr);
 ...
 *  different from that saved in the snapshot, the interrupt associated with
 *  the interrupt cookie (See gxio_mpipe_link_cfg_pollfd()) will be triggered.
 *
 ...
 *  Kernel waiting for a specific link state should check that state
...
extern int gxio_mpipe_link_arm_pollfd(gxio_mpipe_context_t *context, int fd);

/* Configure link status interrupts.
 *
 * @param context An initialized mPIPE context.
 ...
 *  code.
 */
extern int gxio_mpipe_link_cfg_pollfd(gxio_mpipe_context_t *context,
				      int inter_x, int inter_y, int inter_ipi,
				      int inter_event, uint64_t linkmask);

/* Deconfigure link status interrupts.
 *
 * @param context An initialized mPIPE context.
 * @param fd Interrupt cookie returned from gxio_mpipe_link_cfg_pollfd().
 * @return Zero on success, or a negative error code.
 */
extern int gxio_mpipe_link_close_pollfd(gxio_mpipe_context_t *context, int fd);
