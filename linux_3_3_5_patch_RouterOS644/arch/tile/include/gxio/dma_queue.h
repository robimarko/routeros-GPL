...
/* Flags of dma_queue. */
enum dma_queue_flags_e {
	/* Do SW remote buffer return. */
	DMA_QUEUE_EQUEUE_RHWB = 0x01,
};
...
typedef struct {
	 ...
	 */
	void *post_region_addr __attribute__ ((aligned(64)));

	/*
	 * Flags, bits are defined in enum dma_queue_flags_e.
	 */
	uint32_t flags;
...
/* Do remote buffer return.
 *
 * In __gxio_dma_queue_update_credits(), after update of
 * "hw_complete_count", call this function if the dma_queue has
 * DMA_QUEUE_EQUEUE_RHWB bit set.
 * The return value is the new credits.
 * Note the definition of this function is in mpipe.c.
 */
extern uint64_t __gxio_mpipe_remote_buffer_return(__gxio_dma_queue_t
						  *dma_queue,
						  uint64_t
						  orig_hw_complete_count,
						  uint64_t
						  new_hw_complete_count);
