...
/** A flag bit indicating resource allocation on all
    mpipe instances. */
#define HV_MPIPE_ALLOC_MULTI_MPIPES 0x02

/** A flag bit indicating a fixed resource allocation on
    all mpipe instances. */
#define HV_MPIPE_ALLOC_MULTI_MPIPES_FIXED 0x04
...
/** mpipe stat configuration command.
 *  There are some mpipe internal counters to collect dropped packet
 *  count or performance. Use API:
 *      gxio_mpipe_config_stats(gxio_mpipe_context_t* context,
 *                              uint32_t command, uint64_t val)
 *  to configure the collected info. Argument command and val are defined
 *  below.
 */

/** Total stat config command = 3
 */

#define GXIO_MPIPE_STAT_CONFIG_COMM_MAX  3

/** command = GXIO_MPIPE_STAT_CONFIG_COMM_LBL config LBL counter.
 *  Refer to Mpipe MMIO registers MPIPE_LBL_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_COMM_LBL  0x0000

/** val = GXIO_MPIPE_STAT_CONFIG_VAL_LBL will enable
 *  mpipe to collect all packet drop by classifier or loadbalancer.
 *  Refer to Mpipe MMIO registers MPIPE_LBL_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_LBL       0x0


/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_LBL_ONLY will enable
 *  mpipe to collect all packet drop by load balancer ONLY.
 *  Refer to Mpipe MMIO registers MPIPE_LBL_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_LBL_ONLY  0x1

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_LBL_BKT will enable
 *  mpipe to collect all packet drop due to bucket full.
 *  Refer to Mpipe MMIO registers MPIPE_LBL_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_LBL_BKT   0x2

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_LBL_NR will enable
 *  mpipe to collect all packet drop due to notifRing full.
 *  Refer to Mpipe MMIO registers MPIPE_LBL_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_LBL_NR    0x3

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_LBL_PICK will enable the
 * mpipe to counts packets assigned to new NotifRing due to
 * sicky-pick finding full NotifRing(or randomized).
 * Refer to Mpipe MMIO registers MPIPE_LBL_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_LBL_PICK  0x4

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_LBL_PKTS will enable the
 * mpipe to collect packet passed throught the load balancer.
 * Refer to Mpipe MMIO registers MPIPE_LBL_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_LBL_PKTS  0x5


/** val =  GXIO_MPIPE_STAT_CONFIG_COMM_IPKT configs IPKT counter.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_COMM_IPKT  0x0001

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IPKT will enable the
 * mpipe to count packet dropped or truncated due to out of
 * ipkt buffers.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IPKT            0x0

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IPKT_TRUNCATED will enable the
 * mpipe to collect packet truncated due to out of ipkt buffers.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IPKT_TRUNCATED  0x1

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IPKT_DROP will enable
 * the mpipe to collect packet dropped due to out of ipkt
 * buffers.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IPKT_DROP       0x2

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IPKT_PKTS will enable the
 * mpipe to collect packets received from the MAC(s).
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IPKT_PKTS       0x3

/** command =  GXIO_MPIPE_STAT_CONFIG_COMM_IDMA_BE  configus IDMA counter.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_COMM_IDMA     0x0002


/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BE  will enable the
 * mpipe to count packets dropped or truncated due to out of buffer.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA            0x0

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_STALL will enable
 * the mpipe to count number of stall cycles due to waiting for buffers.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_STALL  0x1

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_TLB_STALL will enable
 * the mpipe to count number of stall cycles due to TLB miss.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_TLB_STALL  0x2

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_PKTS will enable
 * the mpipe to count number of packets written.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_PKTS       0x3

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BUFS will enable
 * the mpipe to count number of buffers used.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BUFS       0x4

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_RETRIES will enable
 * the mpipe to count number of retried val =s.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_RETRIES    0x5

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_SDN_PKTS will enable
 * the mpipe to count number of SDN packets sent.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_SDN_PKTS   0x6

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_SDN_AF will enable
 * the mpipe to count number of cycles stalled due to SDN back pressure.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_SDN_AF     0x7

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_TRK_AF will enable
 * the mpipe to count number of cycles stalled due to request tracker
 * back pressure.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_TRK_AF     0x8

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_NTF_AF will enable
 * the mpipe to count number of cycles stalled due to notf-q
 * back pressure.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_NTF_AF     0x9

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_SPILL will enable
 * the mpipe to count number of buffer stack spills.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_SPILL  0xA

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_FILL will enable
 * the mpipe to count number of buffer stack fills.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_FILL   0xB

/** val =  GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_EDMA will enable
 * the mpipe to count number of buffer returned by eDMA.
 * Refer to Mpipe MMIO registers MPIPE_IDMA_CTL
 */

#define GXIO_MPIPE_STAT_CONFIG_VAL_IDMA_BSM_EDMA   0xC
...
#define GXIO_MPIPE_LINK_12G        0x0000000000000080UL
