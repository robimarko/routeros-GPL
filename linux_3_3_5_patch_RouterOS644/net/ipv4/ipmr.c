...
static int ipmr_update_output_devs(struct mr_table *mrt,
	...
	temp = kmalloc(count * sizeof(unsigned), GFP_ATOMIC);
	if (!temp)
		return -ENOMEM;
	...
	if (copy_from_user(temp, opts, count * sizeof(unsigned))) {
		kfree(temp);
		return -EFAULT;
	}
	...
	for (i = 0; i < count; ++i) {
		c->mfc_un.res.output_devs[i] = vif_find(mrt, temp[i]);
	}
	kfree(temp);
